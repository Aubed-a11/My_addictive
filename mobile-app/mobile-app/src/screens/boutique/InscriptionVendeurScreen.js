...
export default function InscriptionVendeurScreen({ navigation }) {
  ...
  const soumettre = async () => {
    ...
    try {
      ...
      setMessage('Demande envoyée. Votre boutique sera visible apres validation par l\'administration.');
    } catch (e) {
      setErreur(e.message);
    } finally {
      setChargement(false);
  ...
  return (
    <EcranCentre piedDePage={<BottomTabBar navigation={navigation} variante="boutique" />}>
      ...
      <TextField label="Catégorie (MODE, HIGH_TECH, MAISON, ...)" value={categorie} onChangeText={setCategorie} />
