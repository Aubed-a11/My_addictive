...
export default function LecteurScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // appel peut echouer sans rapport avec la lecture elle-meme, et ne doit
        // jamais empecher la suite (creation du son) de s'executer.
        try {
        ...
        const { sound } = await Audio.Sound.createAsync(
          ...
          (statut) => {
            if (annule) return;
            if (!statut.isLoaded) {
              if (statut.error) console.warn('[Lecteur] Erreur de lecture audio :', statut.error);
              return;
            }
        ...
        // On journalise neanmoins l'erreur reelle (au lieu de l'avaler en silence)
  ...
  const telecharger = async () => {
    if (telechargement === 'termine') {
      Alert.alert('Déjà telecharge', 'Ce titre est déjà disponible hors-ligne sur cet appareil.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Telechargement terminé', `« ${titre.nom} » est maintenant disponible hors-ligne.`);
    } catch (e) {
  ...
  const ajouterAUneListe = async () => {
    if (!estConnecte) { navigation.navigate('Connexion'); return; }
    if (favori) {
      Alert.alert('Déjà dans vos favoris', `« ${titre.nom} » est deja present dans vos favoris.`);
      return;
    }
  ...
  const afficherParoles = () => {
    Alert.alert('Paroles non disponibles', "Les paroles de ce titre n'ont pas encore été renseignees.");
  };
  ...
  const ouvrirMinuteur = () => {
    if (minuteurActif) {
      ...
      Alert.alert('Minuteur annule', 'La mise en pause automatique a été annulée.');
      return;
    }
  ...
  const aUnVraiSon = sonPret;
  const chargementEnCours = !!titre.fichierAudioUrl && !sonPret;
    ...
    <View style={{ flex: 1 }}>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top', 'bottom']}>
        ...
        <View style={styles.pochetteConteneur}>
          ...
          <View style={styles.badgeHQ}><Text style={styles.badgeHQTexte}>{aUnVraiSon ? 'HQ' : chargementEnCours ? 'CHARGEMENT...' : 'APERCU'}</Text></View>
        ...
        <View style={styles.controles}>
          ...
          <Pressable style={[styles.boutonLecture, !aUnVraiSon && styles.boutonLectureDesactive]} onPress={basculerLecture} disabled={chargementEnCours}>
            {chargementEnCours ? (
