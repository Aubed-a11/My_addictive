            ...
            <Pressable style={styles.carte} onPress={() => navigation.navigate('Candidats', { competitionId: item.id, nom: item.nom })}>
              ...
              {item.dateFinPhase && item.phase !== 'TERMINEE' && (
                <View style={styles.ligneCompteARebours}>
                  ...
                  <CompteARebours dateCible={item.dateFinPhase} style={styles.compteARebours} texteExpire="Phase terminée" />
                </View>
              )}
            </Pressable>
          )}
          ListEmptyComponent={<Text style={styles.vide}>{recherche ? 'Aucun résultat pour cette recherche.' : 'Aucune competition en cours.'}</Text>}
        />
