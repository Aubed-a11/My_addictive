  ...
  const acheter = async () => {
    ...
    try {
      ...
      if (transaction.statut === 'REUSSI') {
        ...
        setMessage("Demande enregistree. Presentez-vous a l'agence pour regler le montant : le titre sera disponible une fois le paiement verifie par un agent.");
      } else if (MOBILE_MONEY.includes(moyenPaiement)) {
        setMessage('Une demande de paiement a ete envoyee sur votre telephone. Validez-la avec votre code PIN Mobile Money : le titre sera disponible des confirmation.');
      } else {
        setMessage('Paiement en cours de verification. Le titre sera disponible des sa confirmation.');
      }
    } catch (e) {
      setErreur(e.message);
    } finally {
