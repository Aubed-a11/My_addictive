...
export default function TitreDetailScreen({ navigation, route }) {
  ...
  const acheter = async () => {
    ...
    try {
      const { data: transaction } = await client.post('/api/musique/achats/initier', {
      ...
      if (transaction.statut === 'REUSSI') {
        ...
        setMessage("Demande enregistree. Presentez-vous a l'agence pour regler le montant : le titre sera disponible une fois le paiement vérifié par un agent.");
      } else if (MOBILE_MONEY.includes(moyenPaiement)) {
        setMessage('Une demande de paiement a été envoyée sur votre téléphone. Validez-la avec votre code PIN Mobile Money : le titre sera disponible des confirmation.');
      } else {
        setMessage('Paiement en cours de vérification. Le titre sera disponible des sa confirmation.');
      }
    } catch (e) {
      setErreur(e.message);
    } finally {
  ...
  const telechargerHorsLigne = async () => {
    ...
    try {
      ...
      setErreur("Échec du telechargement : " + e.message);
    } finally {
          ...
          <>
            ...
            {moyenPaiement === 'AGENCE' && (
              <Text style={styles.avertissementAgence}>
                Le titre ne sera disponible qu'apres verification de votre paiement en agence par un agent.
              </Text>
            )}
            <PrimaryButton
...
const styles = StyleSheet.create({
  ...
  meta: { color: COLORS.texteAtténué, fontSize: 12, marginTop: 10, marginBottom: 8 },
  ...
  libelleMoyenPaiement: { color: '#fff', fontWeight: '600', fontSize: 13, marginTop: 8, marginBottom: 8 },
