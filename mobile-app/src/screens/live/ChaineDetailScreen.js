  ...
  const rejoindreFanClub = async () => {
    ...
    try {
      ...
      if (transaction.statut === 'REUSSI') {
        ...
        setMessage("Demande enregistree. Ton acces fan club s'activera une fois le paiement verifie en agence.");
        setChoixPaiementOuvert(false);
      } else if (MOBILE_MONEY.includes(moyenPaiement)) {
        setMessage('Demande envoyee sur ton telephone. Valide avec ton code PIN pour activer ton acces.');
        setChoixPaiementOuvert(false);
      } else {
