  ...
  const acheterBillet = async () => {
    ...
    try {
      ...
      if (transaction.statut === 'REUSSI') {
        ...
        setMessage("Demande enregistree. Presentez-vous a l'agence pour regler le montant : votre billet sera disponible dans \"Mes billets\" une fois le paiement vérifié par un agent.");
      } else if (MOBILE_MONEY.includes(moyenPaiement)) {
        setMessage('Une demande de paiement a été envoyée sur votre téléphone. Validez-la avec votre code PIN Mobile Money : votre billet apparaitra automatiquement dans "Mes billets" des confirmation.');
      } else {
        setMessage('Paiement en cours de vérification. Votre billet apparaitra dans "Mes billets" des sa confirmation.');
      }
    } catch (e) {
      setErreur(e.message);
    } finally {
