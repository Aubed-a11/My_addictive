...
/**
 * Bandeau defilant en haut de Musique : titres les plus ecoutes du moment
 * (classement streaming), pour mettre en avant ce qui est actuellement
 * populaire.
 */
export default function MusiqueCarousel({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await client.get('/api/musique/classements', { params: { type: 'streaming', page: 0, size: 10 } });
    ...
    <View style={styles.conteneur}>
      <FlatList
        ...
        renderItem={({ item, index: i }) => (
          <Pressable style={[styles.carte, { width: largeurCarte }]} onPress={() => navigation.navigate('Lecteur', { id: item.id })}>
            {item.imageUrl ? (
            ...
            <View style={styles.badge}><Text style={styles.badgeTexte}>TOP {i + 1}</Text></View>
            <View style={styles.texteConteneur}>
              <Text style={styles.carteTitre} numberOfLines={1}>{item.nom}</Text>
              <Text style={styles.carteSousTitre} numberOfLines={1}>{item.artiste}</Text>
            </View>
          </Pressable>
        )}
      />
...
const styles = StyleSheet.create({
  ...
  badge: { position: 'absolute', top: 12, right: 12, backgroundColor: COLORS.musique, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 4 },
  ...
  pastilleActive: { backgroundColor: COLORS.musique, width: 16 },
});
